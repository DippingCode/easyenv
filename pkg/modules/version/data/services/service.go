...
func parseHeader(headerLine string) (number string, date string) {
	re := regexp.MustCompile(`^\s*#{3,6}\s*\[(\d+\.\d+\.\d+)\]\s*[-–—]\s*(\d{4}-\d{2}-\d{2})\s*$`)
...
func extractList(body string, heading string) []string {
	...
	rest := body[loc[1]:]
	stopRE := regexp.MustCompile(`(?m)^#{3,6}\s*(?:[A-Za-z].*|\d+\.\d+\.\d+)\s*$`) // Adjusted regex for stopRE
...
func extractBuildDetails(body string) entities.BuildDetails {
